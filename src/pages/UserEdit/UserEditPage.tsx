...
export default function UserEditPage() {
    ...
    const renderRoleBadge = (roleName: string) => {
        if (roleName === RoleEnum.ADMINISTRATOR) {
            return <span key={roleName} className="px-3 py-1 bg-red-100 text-[#7A1014] text-[10px] font-bold rounded-full tracking-wider">{t('userEdit.roles.adminBadge')}</span>;
        }
        return <span key={roleName} className="px-3 py-1 bg-teal-100 text-teal-800 text-[10px] font-bold rounded-full tracking-wider">{roleName === "TEACHER" ? t('userEdit.roles.teacher').toUpperCase() : roleName === "STUDENT" ? t('userEdit.roles.student').toUpperCase() : roleName}</span>;
    };
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row gap-8 items-start">
                    ...
                    <div className="w-full md:w-2/3 bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                                ...
                                <div>
                                    ...
                                    <div className="relative bg-gray-50 border border-gray-200 rounded-md">
                                        <input
                                            ...
                                            className="w-full bg-transparent p-3 text-sm font-medium text-gray-600 outline-none pr-10 cursor-not-allowed"
                                        />
                                        <Eye size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#7A1014]" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 flex items-center justify-between mb-8 opacity-70">
                            <div>
                            ...
                            <div className="flex items-center gap-2">
                                <div className={`w-10 h-6 flex items-center rounded-full p-1 cursor-not-allowed ${user.account.active ? "bg-gray-300" : "bg-[#7A1014]"}`}>
                                    <div className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-300 ease-in-out ${user.account.active ? "" : "translate-x-4"}`}></div>
                                </div>
                                <span className="text-sm font-bold text-gray-800">{user.account.active ? t('userEdit.blockStatus.active') : t('userEdit.blockStatus.blocked')}</span>
                            </div>
                        </div>
